...
def scrape_history
  ...
  # add data
  # FIXME: this definitely could be faster, see if insert_many will work
  ...
  # $cks_history.update(
  #   { "package": "rgbif", "data.$.date_updated" > thirtydaysago.to_time },
  #   { '$unset':  }
  # )
end
...
def history_update(x)
  $cks_history.update_one({ package: x['package'] }, 
    '$addToSet' => { history: x.slice('summary', 'checks', 'date_updated') })
end
